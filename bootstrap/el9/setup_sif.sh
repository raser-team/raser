...
export RASER_SIF_IMAGE=${RASER_SIF_IMAGE:-$dir_raser/img/raser_el9.sif}
geant4_prefix=${RASER_GEANT4_INSTALL:-${GEANT4_INSTALL:-${GEANT4_DIR:-}}}
if [ -z "$geant4_prefix" ]; then
    echo "Warning from raser SIF setup: set RASER_GEANT4_INSTALL, GEANT4_INSTALL, or GEANT4_DIR" >&2
fi
...
[ -n "$geant4_prefix" ] && raser_sif_ld_library_path=$geant4_prefix/lib64
