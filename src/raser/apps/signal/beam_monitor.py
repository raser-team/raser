...
def get_beam_number(my_g4,ele_current):
    ...
    path = output(__file__, "beam_monitor", now)
    number = array('d',[999.])
    hittotal = array('d',[999.])
    number[0] = int(-ele_current.max_Broad_Band_height/18.8)
    hittotal[0]=my_g4.hittotal
    fout = ROOT.TFile(os.path.join(path, "beam_monitor.root"), "RECREATE")
    ...
    c1.SaveAs(os.path.join(path, "energy.pdf"))
    c1.SaveAs(os.path.join(path, "energy.root"))
