...
class B2TrackerHit(G4VHit):
    ...
    def Print(self):
        ...
        print(
            G4BestUnit(self.fEdep, "Energy"),
            "TruthPosition:",
            G4BestUnit(self.fTruthPos, "Length"),
            "HitPosition:",
            G4BestUnit(self.fHitPos, "Length"),
        )
...
class B2TrackerSD(G4VSensitiveDetector):
    ...
    def ProcessHits(self, aStep, rOhist):
        ...
        print(
            "Jiaqi_B2TrackerSD.ProcessHits()1=",
            "TrackID:",
...
class B2aDetectorConstruction(G4VUserDetectorConstruction):
    ...
    def DefineVolumes(self):
        ...
        print(
            "Computed tolerance =",
            G4GeometryTolerance.GetInstance().GetSurfaceTolerance() / um,
            "um",
        )
        ...
        for j in range(15):
            for i in range(5):
                ...
                if copyNo < 25:
                    ...
                    chamberS = G4Box(
                        "Chamber_solid", SiXSpacing / 2, SiYSpacing / 2, SiZ
                    )

                    self.fLogicChamber += [
                        G4LogicalVolume(
                            chamberS,
                            self.fChamberMaterial,
                            "Chamber_LV",
                            None,
                            None,
                            None,
...
class B2SteppingAction(G4UserSteppingAction):
    ...
    def UserSteppingAction(self, aStep):
        ...
        if aStep.GetTrack().GetTrackID() == 1:
            PreStepVolume = (
                aStep.GetPreStepPoint().GetTouchable().GetVolume().GetLogicalVolume()
            )
                ...
                for j in range(15):
                    for i in range(5):
                        ...
                        if k < 25 and PostStepVolume == Chamberlist[k]:
                            self.hitpos0 = G4ThreeVector(
                                firstXPosition + i * SiXSpacing,
                                firstYPosition + j * SiYSpacing,
                                firstZPosition,
                            )
                        ...
                        if k > 24 and k < 50 and PostStepVolume == Chamberlist[k]:
                            self.hitpos1 = G4ThreeVector(
                                firstXPosition + i * SiXSpacing,
                                firstYPosition + (j - 5) * SiYSpacing,
                                firstZPosition + chamberSpacing / 2,
                            )
                            self.pos1 = aStep.GetPostStepPoint().GetPosition()
                            return self.hitpos1 and self.pos1

                        if k > 49 and PostStepVolume == Chamberlist[k]:
                            self.hitpos2 = G4ThreeVector(
                                firstXPosition + i * SiXSpacing,
                                firstYPosition + (j - 10) * SiYSpacing,
                                firstZPosition + chamberSpacing,
                            )
        ...
        if self.hitpos0 != None and self.hitpos1 != None and self.hitpos2 != None:
            ...
            print(
                "hitpos0:",
                G4BestUnit(self.hitpos0, "Length"),
                "hitpos1:",
                G4BestUnit(self.hitpos1, "Length"),
                "hitpos2:",
                G4BestUnit(self.hitpos2, "Length"),
                "\n位置分辨:X:",
                G4BestUnit(PosResX, "Length"),
                "±",
                G4BestUnit(SiXSpacing, "Length"),
                "Y:",
                G4BestUnit(PosResY, "Length"),
                "±",
                G4BestUnit(SiYSpacing, "Length"),
                "Z:",
                G4BestUnit(PosResZ, "Length"),
            )

            print(
                "pos0:",
                G4BestUnit(self.pos0, "Length"),
                "pos1:",
                G4BestUnit(self.pos1, "Length"),
                "pos2:",
                G4BestUnit(self.pos2, "Length"),
                "\n准位置分辨:",
                G4BestUnit((self.pos0 + self.pos2) / 2 - self.pos1, "Length"),
            )

            points = [
                [self.hitpos0.getX(), self.hitpos0.getY(), self.hitpos0.getZ()],
            ...
            ReconPosResX0 = (
                k1 * (firstZPosition + chamberSpacing / 2) + b1 - self.hitpos1.getX()
            )
            ReconPosResY0 = (
                k2 * (firstZPosition + chamberSpacing / 2) + b2 - self.hitpos1.getY()
            )

            print(
                "重建后位置分辨:X:",
                G4BestUnit(ReconPosResX, "Length"),
                "Y:",
                G4BestUnit(ReconPosResY, "Length"),
                "X0:",
                G4BestUnit(ReconPosResX0, "Length"),
                "Y0:",
                G4BestUnit(ReconPosResY0, "Length"),
            )
...
def main(batchMac=None):
    ...
    if ui == None:
        ...
        UImanager.ApplyCommand(
            "/control/execute {}".format(component_path("g4macro", fileName))
        )
    else:
        # interactive mode
        UImanager.ApplyCommand(
            "/control/execute {}".format(
                component_path("g4macro", "init_vistelescope.mac")
            )
        )
        if ui.IsGUI():
            UImanager.ApplyCommand(
                "/control/execute {}".format(component_path("g4macro", "gui.mac"))
            )
        ui.SessionStart()
