...
class DetectorConstruction(g4b.G4VUserDetectorConstruction):
    def __init__(self, g4_dic):
        ...
        for object_type in g4_dic["object"]:
            ...
            if object_type == "binary_compounds":
                for every_object in g4_dic["object"][object_type]:
                    self.create_binary_compounds(
                        g4_dic["object"][object_type][every_object]
                    )
    ...
    def create_world(self, world_type, world_size):
        ...
        self.solid["world"] = g4b.G4Box(
            "world", world_size * g4b.um, world_size * g4b.um, world_size * g4b.um
        )
    ...
    def create_binary_compounds(self, object):
        ...
        if not "tub" in object:
            ...
            self.solid[name + "box"] = g4b.G4Box(
                name + "box", sidex / 2.0, sidey / 2.0, sidez / 2.0
            )
            self.solid[name + "tub"] = g4b.G4Tubs(
                name + "tub",
                0,
                object["tub"]["tub_radius"] * g4b.um,
                object["tub"]["tub_depth"] * g4b.um,
                0,
                360 * g4b.deg,
            )
            self.solid[name] = g4b.G4SubtractionSolid(
                name, self.solid[name + "box"], self.solid[name + "tub"]
            )

        self.logical[name] = g4b.G4LogicalVolume(self.solid[name], compound, name)
        self.physical[name] = g4b.G4PVPlacement(
            None,
            translation,
            name,
...
class ActionInitialization(g4b.G4VUserActionInitialization):
    def __init__(
        self, detConstruction, par_in, par_direction, par_type, par_energy, par_num
    ):
        super().__init__()
    ...
    def GeneratePrimaries(self, event):
        for i in range(self.par_num):
            ...
            beam.SetParticleMomentumDirection(
                g4b.G4ThreeVector(par_direction[0], par_direction[1], par_direction[2])
            )
...
class EventAction(g4b.G4UserEventAction):
    ...
    def BeginOfEventAction(self, event):
        self.edep_device = 0.0
        self.event_angle = 0.0
        self.p_step = []
        self.energy_step = []
