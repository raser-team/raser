...
class Amplifier:
    ...
    def __init__(self, currents: list[ROOT.TH1F], amplifier_name: str, seed = None, CDet = None, is_cut = False):
        ...
        if os.path.exists(ele_json) and not use_spice_amplifier:
            ...
            for i in range(self.read_ele_num):
                subprocess.run(["ngspice", "-b", tmp_cirs[i]])
    ...
    def draw_waveform(self, currents, path):
        for i in range(self.read_ele_num):
            ...
            c.SaveAs(fig_name)
            c.SaveAs(root_name)
            self.write_raw_waveforms(
                root_name,
                currents[i],
                self.amplified_currents[i],
                scale_factor,
            )

    def write_raw_waveforms(self, root_name, current, amplified_current, scale_factor):
        root_file = ROOT.TFile(root_name, "UPDATE")
        if not root_file or root_file.IsZombie():
            raise OSError("Cannot update ROOT waveform file: {}".format(root_name))
        try:
            raw_current = current.Clone("current_A")
            raw_current.SetTitle("input current;Time [s];Current [A]")
            raw_current.SetDirectory(root_file)
            raw_current.Write("", ROOT.TObject.kOverwrite)

            raw_amplified = amplified_current.Clone("electronics_mV")
            raw_amplified.SetTitle("electronics output;Time [s];Amplitude [mV]")
            raw_amplified.SetDirectory(root_file)
            raw_amplified.Write("", ROOT.TObject.kOverwrite)

            scale_note = ROOT.TNamed(
                "electronics_display_scale_factor",
                str(float(scale_factor)),
            )
            scale_note.Write("", ROOT.TObject.kOverwrite)
        finally:
            root_file.Close()
