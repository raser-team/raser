...
def _resolve_gdml_path(path_text: str) -> str:
    ...
    setting_root = os.getenv('RASER_SETTING_PATH', '')
    candidates = []
    if setting_root:
        candidates.append(os.path.join(setting_root, path_text))
        candidates.append(os.path.join(setting_root, 'g4experiment', path_text))
    repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..'))
    candidates.append(os.path.join(repo_root, path_text))
    candidates.append(os.path.join(repo_root, 'setting', path_text))
